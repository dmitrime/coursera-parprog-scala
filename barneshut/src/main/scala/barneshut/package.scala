...
package object barneshut {
  ...
  case class Fork(
    ...
    val size: Float = math.max(math.max(ne.size, nw.size), math.max(sw.size, se.size))
    ...
    def insert(b: Body): Quad = {
      ...
      } else if (contains(b, ne)) {
        Fork(nw, ne.insert(b), sw, se)
      } else if (contains(b, sw)) {
        Fork(nw, ne, sw.insert(b), se)
      } else if (contains(b, se)) {
    ...
    private def contains(b: Body, q:Quad):Boolean = {
      val halfSize = size / 2
  ...
  extends Quad {
    val mass = bodies.foldLeft(0f)((x,y) => x + y.mass)
    val (massX, massY) = (bodies.foldLeft(0f)((x,y) => x + y.x*y.mass) / mass,
                          bodies.foldLeft(0f)((x,y) => x + (y.y*y.mass)) / mass)
    val total: Int = bodies.size
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val halfSize = size / 2
        val qsize = size / 4
        var quad: Quad = Fork(Empty(centerX - qsize, centerY - qsize, halfSize),
                              Empty(centerX + qsize, centerY - qsize, halfSize),
                              Empty(centerX - qsize, centerY + qsize, halfSize),
                              Empty(centerX + qsize, centerY + qsize, halfSize))
        (bodies :+ b).foldLeft(quad){(q, b) => q.insert(b)}
      }
      else
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = if (b.x < boundaries.minX) 0
              else if (b.x >= boundaries.maxX) sectorPrecision - 1
              else (b.x - boundaries.minX) / sectorSize
      val y = if (b.y < boundaries.minY) 0
              else if (b.y >= boundaries.maxY) sectorPrecision - 1
              else (b.y - boundaries.minY) / sectorSize
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      val m = new SectorMatrix(boundaries, sectorPrecision)
      for (i <- 0 until matrix.length) 
        m.matrix(i) = matrix(i).combine(that.matrix(i))
      m
    }
